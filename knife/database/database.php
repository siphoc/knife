...
class KnifeDatabase
{
	...
	private function connect()
	{
		...
		if(!$this->handler)
		{
			...
			{
				throw new Exception('A database connection could not be established.', 0);
			}
		}
	...
	public function getEnumValues($table, $field)
	{
		...
		if(!isset($row['Type'])) throw new Exception('There is no type information available about this field', 0, $this->password);

		// has a type but it's not an enum
		if(strtolower(substr($row['Type'], 0, 4) != 'enum')) throw new Exception('This field "' . $field . '" is not an enum field.', 0, $this->password);
	...
	public function getPairs($query, $parameters = array())
	{
		...
		foreach($tmpResults as $result)
		{
			...
			if(!isset($keys))
			{
				...
				if(count($keys) != 2) throw new Exception('You have to fetch 2 columns when using getPairs.');
			}
	...
	public function insert($table, array $values)
	{
		...
		// array has values
		if(count($values) == 0) throw new Exception('You need to provide values for an insert query.', 0, $this->password);
		...
		if(is_array($actualValues[0]))
		{
			...
			foreach($values as $aRow)
			{
				...
				if(count($aRow) != $numFields) throw new Exception('Each record of this array should contain the same number of keys.', 0, $this->password);
	...
	private function setDriver($driver)
	{
		...
		// validate backend
		if(!in_array($driver, PDO::getAvailableDrivers())) throw new Exception('The PDO database driver "' . (string) $driver . '" is not found. Only ' . implode(', ', PDO::getAvailableDrivers()) . ' are currently installed.');
	...
	public function update($table, array $values, $where = null, $parameters = array())
	{
		...
		// values check
		if(count($values) == 0) throw new Exception('No values provided.', 0, $this->password);
